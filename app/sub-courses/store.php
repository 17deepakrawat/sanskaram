...
if($_POST['university_id'] == 48){
  ...
  if(isset($_POST['name']) && isset($_POST['course']) && isset($_POST['university_id']) && isset($_POST['scheme']) || isset($_POST['mode']) && isset($_POST['min_duration']) && isset($_POST['max_duration'])){
    ...
    $name = mysqli_real_escape_string($conn, $_POST['name']);
    $short_name = mysqli_real_escape_string($conn, $_POST['short_name']);
    $in_house_course_name = mysqli_real_escape_string($conn, $_POST['in_house_course_name']);
    $scheme = intval($_POST['scheme']);
    $mode = intval($_POST['mode']);
    ...
    if(empty($name) || empty($short_name) || empty($course) || empty($university_id) || empty($scheme) || empty($mode) || empty($eligibilities) || empty($in_house_course_name)){
      echo json_encode(['status'=>403, 'message'=>'All fields are mandatory!']);
      exit();
    }
    ...
    $add = $conn->query("INSERT INTO `Sub_Courses`(`Name`, `Short_Name`, `Course_ID`, `University_ID`, `Scheme_ID`, `Mode_ID`, `Min_Duration`, `SOL`, `Lateral`, `LE_Start`, `LE_SOL`, `Credit_Transfer`, `CT_Start`, `CT_SOL`, `Eligibility`,`in_house_course_name`) VALUES ('$name', '$short_name', $course, $university_id, $scheme, $mode, $min_duration, $max_duration, $lateral, '$le_start', $le_sol, $ct_transfer, $ct_start, $ct_sol, '".json_encode($eligibilities)."','$in_house_course_name')");
    if($add){
