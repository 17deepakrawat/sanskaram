...
if (isset($_POST['name']) && isset($_POST['course']) && isset($_POST['university_id']) && isset($_POST['scheme']) || isset($_POST['mode']) && isset($_POST['min_duration']) && isset($_POST['max_duration']) && isset($_POST['id'])) {
  ...
  if (empty($name) || empty($short_name) || empty($course) || empty($university_id) || empty($scheme) || empty($mode) || empty($id) || empty($in_house_course_name)) {
  ...
  $update = $conn->query("UPDATE `Sub_Courses` SET `Name` = '$name', `Short_Name` = '$short_name', `Course_ID` = $course, `Scheme_ID` = $scheme, `Mode_ID` = $mode, `Min_Duration` = '" . json_encode($min_duration) . "', `SOL` = $max_duration, `Lateral` = $lateral, `LE_Start` = '$le_start', `LE_SOL` = $le_sol, `Credit_Transfer` = $ct_transfer, `CT_Start` = $ct_start, `CT_SOL` = $ct_sol, Eligibility = '" . json_encode($eligibilities) . "',`in_house_course_name`='$in_house_course_name' WHERE ID = $id");
  if ($update) {
