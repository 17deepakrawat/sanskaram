...
if (isset($_POST['university_id']) && isset($_POST['id']) && isset($_POST['counsellor'])) {
  ...
  // $sub_course_status = array_filter($sub_course_status);
    ...
    foreach ($fees as $sub_course_id=>$value){
      ...
      foreach($value as $duration=>$fee){
        ...
        if(empty($fee)){
          ...
          $conn->query("DELETE FROM Sub_Center_Sub_Courses WHERE User_ID = $id AND Duration = '$duration' AND Sub_Course_ID = $sub_course_id");
          $alloted = $conn->query("INSERT INTO Sub_Center_Sub_Courses (`Fee`, `User_ID`, `Duration`, `Sub_Course_ID`, `Course_ID`, `University_ID`,`Sub_Courses_Status`) VALUES ($fee, $id, '$duration', $sub_course_id, $courseId, $university_id, $sub_course_status_id)");
        }
      }
    }
