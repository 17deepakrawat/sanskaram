...
function getAdmissionSession() : string {
    ...
    $searchQuery = '';
    $searchQuery .= !empty($scheme_filter) ?  " AND Scheme_ID IN ('".$scheme_filter."')" : "";
...
function getCourse() : string {
    ...
    $searchQuery = '';
    $searchQuery .= !empty($scheme_filter) ?  " AND Scheme_ID IN ('".$scheme_filter."')" : "";
    $coursesQuery = "SELECT ID,Name FROM `Sub_Courses` WHERE University_ID = '".$_SESSION['university_id']."' $searchQuery";
...
function getDuration() : string {
    ...
    if ($_SESSION['university_id'] == 48) {
        $durations = $conn->query("SELECT DISTINCT CONCAT(Duration,'(',Course_Category,')') as `duration` FROM `Students` WHERE University_ID = '48' AND Duration <> ''");
        $durations = mysqli_fetch_all($durations,MYSQLI_ASSOC);
        $durations_details = array_column($durations,'duration','duration');
        $option .= getOptionTagData($durations_details,$option);
    } else if ($_SESSION['university_id'] == 47) {
        $searchQuery = (!empty($course_filter)) ? " AND ID IN (".$course_filter.")" : "";
        $maxDuration = $conn->query("SELECT MAX(CAST(TRIM('\"' FROM Min_Duration) AS int)) FROM Sub_Courses WHERE University_ID = '47' $searchQuery");        
...
function getStudent() : string {
    ...
    $searchQuery = '';
    $searchQuery .= !empty($scheme_filter) ?  " AND Admission_Sessions.Scheme_ID IN (".$scheme_filter.")" : "";
