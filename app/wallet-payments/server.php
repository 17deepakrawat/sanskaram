...
$rowperpage = ($rowperpage == "all") ? $totalRecords : $rowperpage;
...
## Fetch records
$result_record = "SELECT Wallets.ID,Wallets.Added_By as user_id,  Wallets.Transaction_Date, Wallets.Type, Wallets.Transaction_ID, Wallets.Gateway_ID, Wallets.Amount, Wallets.File, Wallets.Payment_Mode, Wallets.Bank, Wallets.Added_For, Wallets.Added_By, Wallets.Status, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as University FROM Wallets LEFT JOIN Universities ON Wallets.University_ID = Universities.ID LEFT JOIN Students ON Wallets.Added_For = Students.ID WHERE (Type = 1 OR Type = 2) $university_query $searchQuery $role_query $orderby LIMIT " . $row . "," . $rowperpage;
