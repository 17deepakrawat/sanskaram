  ...
  if(isset($_GET['university_id']) && isset($_GET['session_id']) && isset($_GET['admission_type_id']) && isset($_GET['course_id']) && isset($_GET['center'])){
    ...
    if($is_vocational->num_rows>0){
      ...
      if($sub_course_ids->num_rows == 0){
        $sub_course_ids = $conn->query("SELECT DISTINCT Sub_Course_ID FROM Sub_Center_Sub_Courses WHERE `User_ID` = $user_id AND University_ID = $university_id AND Course_ID = $course_id AND Fee > 0  AND Sub_Courses_Status = 0");
      }
