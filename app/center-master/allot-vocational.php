  ...
  if(isset($_POST['university_id']) && isset($_POST['id']) && isset($_POST['counsellor']) && isset($_POST['course_type'])){
    ...
    if($university_id == 48){ 
      ...
      foreach ($fees as $sub_course_id=>$value){
        ...
        foreach($value as $duration=>$fee){
          ...
          if(empty($fee)){
            ...
            $conn->query("DELETE FROM Center_Sub_Courses WHERE User_ID = $id AND Duration = '$duration' AND Sub_Course_ID = $sub_course_id");
            $allot = $conn->query("INSERT INTO Center_Sub_Courses (`Fee`, `User_ID`, `Duration`, `Sub_Course_ID`, `Course_ID`, `University_ID`,`Sub_Courses_Status`) VALUES ($fee, $id, '$duration', $sub_course_id, $courseId, $university_id, $sub_course_status_id )");
          }
        }
      }
    }else{
      ...
      foreach ($fees as $sub_course_id=>$fee){
        ...
        $sub_course_status_id = !empty($sub_course_status[$sub_course_id])?$sub_course_status[$sub_course_id] :0;
        $allot = $conn->query("INSERT INTO Center_Sub_Courses (`Fee`, `Duration`, `User_ID`, `Course_ID`, `Sub_Course_ID`, `University_ID`,`Sub_Courses_Status`) VALUES ($fee, 1, $id, $course_id, $sub_course_id, $university_id, $sub_course_status_id)");
      }
    }
