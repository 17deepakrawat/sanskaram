...
?>
  ...
  <div class="modal-body">
    ...
    <?php if(!empty($alloted)){ 
      ...
      <div class="row">
        <?php
          ...
          $universities = $conn->query("SELECT ID, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as Name, Logo FROM Universities WHERE Universities.Status=1 $alloted_query");
    ...
    <?php $not_alloted_query = !empty($alloted) ? " AND ID NOT IN (".implode(',', $alloted).")" : "";
      $universities = $conn->query("SELECT ID, CONCAT(Universities.Short_Name, ' (', Universities.Vertical, ')') as Name, Logo FROM Universities WHERE Universities.Status=1 $not_alloted_query");
