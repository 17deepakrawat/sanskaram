...
## Fetch records
 $result_record = "SELECT `ID`, `Name`,Vertical_type, `CanCreateSubCenter`, `Email`,`Created_At`, `Code`, `Status`, `Photo`, CAST(AES_DECRYPT(Password, '60ZpqkOnqn0UQQ2MYTlJ') AS CHAR(50)) password FROM Users WHERE Role = 'Center' $center_query $searchQuery $searchQuerySql $orderby LIMIT ".$row.",".$rowperpage;
$empRecords = mysqli_query($conn, $result_record);
$data = array();
// echo $result_record; die;
while ($row = mysqli_fetch_assoc($empRecords)) {
  $sub_center = $conn->query("SELECT GROUP_CONCAT(Sub_Center) , COUNT(Sub_Center) AS total_count FROM `Center_SubCenter` WHERE Center= '" . $row['ID'] . "'");
  ...
  $sub_center_count = isset($sub_centers['total_count']) ? $sub_centers['total_count'] : 0;
