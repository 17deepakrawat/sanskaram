...
$filter_count = $conn->query("SELECT COUNT(ID) as filtered FROM `MarkSheet_Entry` $searchQuery");
$records = mysqli_fetch_assoc($filter_count);
$totalRecordwithFilter = $records['filtered'];

$data = [];
## Fetch records
$marksheetRecords = $conn->query("SELECT MarkSheet_Entry.* , CONCAT(MarkSheet_Entry.Duration,'/',Students.Course_Category) as 'duration_concat' , Students.Unique_ID as `Student_id`  FROM `MarkSheet_Entry` LEFT JOIN Students ON Students.Enrollment_No = MarkSheet_Entry.Enrollment_No $searchQuery $orderby LIMIT $row, $rowperpage");
