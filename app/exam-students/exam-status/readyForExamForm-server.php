    ...
    if (!empty($student_ids)) {
      ...
      if (isset($_REQUEST['vertical']) ) {
        if ($_REQUEST['vertical'] == '0') {
          ...
          $searchQuery .= " AND Users.Vertical_type = '$verticalTypeId'";
...
function examAppearListOfStudent($month, $year): array
{
  ...
  $student_record_query = "SELECT ID,Enrollment_No,Admission_Session_ID,Duration,Unique_ID FROM `Students` WHERE University_ID = '48' AND Enrollment_No IS NOT NULL $role_query";
  ...
  if ($student_records->num_rows > 0) {
    ...
    while ($student_record = mysqli_fetch_assoc($student_records)) {
      if (array_key_exists($student_record['Admission_Session_ID'], $adm_month_year)) {
        ...
        date_modify($studentExamSession, "+" . ($duration - 1) . "Month");
