...
if (isset($_GET['course_id']) && !empty($_GET['course_id']) && isset($_GET['semester']) && !empty($_GET['semester'])) {
  ...
  $syllabus_ids = array();
  $codes = $conn->query("SELECT ID FROM Syllabi WHERE Sub_Course_ID = " . $sub_course_id . " AND Semester = '" . $semester . "' AND Scheme_ID = " . $scheme . " AND University_ID = " . $_SESSION['university_id']);

  if ($codes->num_rows > 0) {
    while ($row = $codes->fetch_assoc()) {
      $syllabus_ids[] = $row['ID'];
    }
    $date_sheets = $conn->query("SELECT Date_Sheets.*, Exam_Sessions.Name as Exam_Session, Syllabi.Name, Syllabi.Code FROM Date_Sheets LEFT JOIN Syllabi ON Date_Sheets.Syllabus_ID = Syllabi.ID LEFT JOIN Exam_Sessions ON Date_Sheets.Exam_Session_ID = Exam_Sessions.ID WHERE   Scheme_ID = $scheme AND  Syllabus_ID IN (" . implode(",", $syllabus_ids) . ") ORDER BY Exam_Date ASC");

    if ($date_sheets->num_rows == 0) { ?>
      <tr>
        <td colspan="5" style="text-align: center;">Date Sheet Not Available
      <tr>
      <?php } else {
    ...
    <tr>
      <td colspan="5" style="text-align: center;">NO Syllebus Available</td>
    </tr>
  <?php } ?>
<?php } ?>