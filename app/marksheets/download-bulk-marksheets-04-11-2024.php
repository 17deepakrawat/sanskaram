...
if ($student->num_rows > 0) {
    while ($row = $student->fetch_assoc()) {
       ...
       $exam_date = $conn->query("SELECT m.exam_month,m.exam_year FROM marksheets AS m LEFT JOIN Syllabi AS s ON m.subject_id = s.ID WHERE m.enrollment_no = '" . $data['Enrollment_No'] . "' AND s.Course_ID = " . $data['Course_ID'] . "  AND  s.Sub_Course_ID = " . $data['Sub_Course_ID'] . " $sem_sql  GROUP BY m.enrollment_no");
        ...
        $temp_subjects = $conn->query("SELECT marksheets.exam_month,marksheets.exam_year, marksheets.obt_marks_ext, marksheets.obt_marks_int,marksheets.obt_marks,marksheets.status,marksheets.remarks,marksheets.Created_At,Syllabi.Code,Syllabi.Name as subject_name,Syllabi.Min_Marks, Syllabi.Max_Marks FROM marksheets LEFT JOIN Syllabi ON marksheets.subject_id = Syllabi.ID WHERE enrollment_no = '" . $data['Enrollment_No'] . "' AND Sub_Course_ID = '".$row['Sub_Course_ID']."'  $sem_sql ORDER BY Syllabi.Code ASC");
        $data['marks'] = array();
        $temp_subject = [];
        $total_obt = 0;
        $total_max = 0;
        ...
        if ($temp_subjects->num_rows > 0) {
            ...
            if ($data['University_ID'] == 48) {
                ...
                $pdf->SetXY(43, 110);
                $pdf->MultiCell(68, 10, 'Subject Name ', 'TLB', 'C');
                $pdf->SetXY(111, 110);
                ...
                foreach ($data['marks'] as $mark) {
                    ...
                    if ($mark['obt_marks_ext'] != 'AB' && $mark['obt_marks_ext'] != 'AB') {
                        $ext_marks = ($mark['obt_marks_ext']=='AB') ? 0 :$mark['obt_marks_ext'];
                        $int_marks = ($mark['obt_marks_int']=='AB') ? 0 :$mark['obt_marks_int'];
                        $total_get_marks = $ext_marks + $int_marks;
                    }else{
                        $total_get_marks ='AB';
                        $mark['remarks'] = 'FAIL';
                    }
                    ...
                    if (strlen($mark['subject_name']) > 30) {
                        ...
                        $pdf->MultiCell(68, 5, $nameParts[0] . chr(10) . $nameParts[1], 'LB', 0, 0, 'L');
                        $x = $pdf->GetX();
                        $y = $pdf->GetY();
                        $pdf->SetXY($x + 101, $y - 10);
                        $pdf->Cell($cellWidth, $cellHeight - 10, $total_get_marks , 'LB', 0, 'C');
                        ...
                        $pdf->Cell(68, $cellHeight, trim($mark['subject_name']), 'LB', 0, 'L');
                        $pdf->Cell($cellWidth, $cellHeight, $total_get_marks , 'LB', 0, 'C');
                ...
                if ($count >= 9) {
                    ...
                    if (in_array('FAIL', $remark_status)) {
                        ...
                        $pdf->SetXY(116, 246.6);
                        $pdf->Cell(35, 8, "FAIL", 'LR', 1, 'C', 0);
                        $pdf->SetXY(151, 246.6);
                        $pdf->Cell(42, 8, '', 'R', 1, 'C', 0);
                    } else {
                        $pdf->SetXY(116, 246.6);
