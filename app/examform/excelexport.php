   ...
   LEFT JOIN Admission_Sessions ON Students.Admission_Session_ID = Admission_Sessions.ID
        LEFT JOIN Courses ON Students.Course_ID = Courses.ID
            LEFT JOIN Universities ON Students.University_ID = Universities.ID
                LEFT JOIN Examination_Confirmation ON Examination_Confirmation.Student_ID = Students.ID WHERE Examination_Confirmation.Confirmation_Status=1 AND Students.University_ID= '".$_SESSION['university_id']."'
...
$result_record = "SELECT CONCAT_WS(' ', Students.First_Name, Students.Middle_Name, Students.Last_Name) AS student_name,
    Students.Enrollment_No,Students.Duration,Students.Course_Category,
...
WHERE Examination_Confirmation.Confirmation_Status = 1  AND Students.University_ID= '".$_SESSION['university_id']."'
...
while ($row = mysqli_fetch_assoc($results)) {
  if($_SESSION['university_id']==48){
    if (is_numeric($row['Duration'])) {
            $category_type = ($row['Course_Category'] == 'advance_diploma') ? 'advanced(advance_diploma)' : $row['Course_Category'];
            $due_val = $row['Duration'] . '/' . $category_type;
        } else {
            $due_val = ($row['Duration'] == '11/advance-diploma') ? '11/advanced(advance-diploma)' : $row['Duration'];
        }
  }else{
     $due_val = $row['Duration'];
  }
    $data[] = array(
        "student_name" => $row["student_name"],
        "enrollment_no" => $row["Enrollment_No"],
        "sub_course_name" => $row["sub_course_name"],
        "semester" => $due_val ,//$row["semester"],
