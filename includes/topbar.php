  ...
  <div class="">
    <div class="brand inline">
      <?php if ($_SESSION['Role'] != 'Sub-Center') {
        if (!empty($dark_logo)) { ?>
          <img src="<?= $dark_logo ?>" alt="logo" data-src="<?= $dark_logo ?>" data-src-retina="<?= $dark_logo_retina ?>"
            height="35" style="max-width:120px">
        <?php }
        ...
        if ($logo->num_rows > 0) {
          ...
          ?>
          <img src="<?= $logo['Photo'] ?>" alt="center_logo" data-src="<?= $logo['Photo'] ?>"
            data-src-retina="<?= $logo['Photo'] ?>" height="35">
        <?php }
      } ?>
    </div>
      ...
      <ul class="d-lg-inline-block notification-list no-margin d-lg-inline-block b-grey b-l no-style p-l-20 p-r-20">
        <li class="p-r-5 inline">
          <a href="javascript:;" id="notification-center" class="header-icon" "<?php if ($_SESSION['Role'] == 'Administrator' || (isset($_SESSION['Alloted_Universities']) && count($_SESSION['Alloted_Universities']) > 1)):
            ...
            <img src="<?= $_SESSION['university_logo'] ?>" alt="logo" data-src="<?= $_SESSION['university_logo'] ?>"
              data-src-retina="<?= $_SESSION['university_logo'] ?>" height="42px">
          </a>
        </li>
      </ul>
    ...
    <div class="dropdown pull-right">
      <button class="profile-dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true"
        ...
        <span class="thumbnail-wrapper d32 circular inline">
          <img src="<?= $_SESSION['Photo'] ?>" alt="" data-src="<?= $_SESSION['Photo'] ?>"
            data-src-retina="<?= $_SESSION['Photo'] ?>" width="32" height="32">
        </span>
      </button>
      <div class="dropdown-menu dropdown-menu-right profile-dropdown" role="menu">
        <a href="#" class="dropdown-item"><span>Signed in as
            <br /><b><?= ucwords(strtolower($_SESSION['Name'])) ?></b></span></a>
