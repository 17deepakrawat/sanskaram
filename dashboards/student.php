        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="tile-progress tile-cyan">
              ...
              <div class="tile-footer">
                <h5 class="mb-0"> <a href="/student/examination/date-sheets" class="text-white">See Datesheet <i class="ti-arrow-right ml-2"></i></a> </h5>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            ...
            $data = array();
              while ($row = $result_record->fetch_assoc()) { ?>
                <tr>
                  ...
                  <td class="text-center"><a type="btn btn-link" class="text-primary" onclick="view_content('<?= $row['ID'] ?>');"><i class="fa fa-eye">View</i></a></td>
                  <td>
...
function studentSearchQuery() : string {
  ...
  if ($university_id == '48') {
      ...
      $Course_Category = mysqli_real_escape_string($conn,$_SESSION['Course_Category']);
      $combine_duration = $duration . '(' . $Course_Category . ')';
  } else {
      $duration = mysqli_real_escape_string($conn,$_SESSION['Duration']);
      $combine_duration = $duration;
  }

  $searchQuery .= "AND IF(Notifications_Generated.duration != '',JSON_UNQUOTE(JSON_EXTRACT(Notifications_Generated.duration,'$.{$sub_course_id}')) LIKE '%{$combine_duration}%',true)";
