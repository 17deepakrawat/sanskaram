            ...
            <div class="pull-right">
              <div class="row">
                <div class="col-xs-7" style="margin-right: 10px;">
                  <input type="text" id="e-book-search-table" class="form-control pull-right p-2 fw-bold"
                    placeholder="Search">
                </div>
      ...
      var settings = {
        ...
        },
        {
          ...
          render: function (data, type, row) {
            ...
            // alert(data);
            var color = (data == "SUBMITTED") ? "green" : "red";
            return '<span style="font-weight: 500; color: ' + color + '">' + data.charAt(0).toUpperCase() + data.slice(1).toLowerCase() + '</span>';
        ...
        },
        {
          ...
          render: function (data, type, row) {
            ...
            if (row.File_Type && row.File_Type.toLowerCase() === 'pdf' && row.uploadingIsActive == 1) {
              file = '<a href="' + path + data + '" class="btns btn-dangers btn-sm" download aria-label="Download Assignment" data-toggle="tooltip" data-placement="top" title="Download Assignment"  > <i class="uil uil-down-arrow"></i></a>';
            } else {
              file = '<a href="' + path + data + '"  class="btns btn-dangers btn-sm" download aria-label="Download Assignment" data-toggle="tooltip" data-placement="top" title="Download Assignment" > <i class="uil uil-down-arrow"></i></a>';
            }
            return file;
...

        {
          ...
          render: function (data, type, row) {
            ...
            if (row.uploadingIsActive === 0) {
              ...
              fileLinks += '<a href="' + zipLink + '" class="btns btn-dangers btn-sm" download aria-label="Download Submitted Assignment" data-toggle="tooltip" data-placement="top" title="Download Submitted Assignment"  > <i class="uil uil-down-arrow"></i> </a> ';
              button = '<a class="btns btn-successs btn-sm" data-toggle="modal" onclick=\'openUploadModal("' + row.id + '", "' + row.subject_id + '")\' aria-label="Re-Upload Assignment" data-toggle="tooltip" data-placement="top" title="Re-Upload Assignment"   ><i class="uil uil-upload"></i> </a>'; //kp


            } else if (row.assignment_submission_status.trim() == "NOT SUBMITTED" && row.uploadingIsActive == 1) {

              button = '<a class="btns btn-primarys btn-sm" data-toggle="modal" onclick=\'openUploadModal("' + row.id + '", "' + row.subject_id + '")\' data-toggle="modal" aria-label="Upload Assignment" data-toggle="tooltip" data-placement="top" title="Upload Assignment"  > <i class="uil uil-upload"></i></a>';
            }
