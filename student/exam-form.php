...
function checkStudentStatus() : array {
    ...
    global $conn;
    $studentAdmission = $conn->query("SELECT Admission_Sessions.Name as `name` , Students.Duration as `duration` FROM `Students` LEFT JOIN Admission_Sessions ON Admission_Sessions.ID = Students.Admission_Session_ID WHERE Students.ID = '". $_SESSION['ID']."'");
